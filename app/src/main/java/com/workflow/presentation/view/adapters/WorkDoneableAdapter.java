...
public class WorkDoneableAdapter extends PaginationAdapter<DoneWorkListModel, OnRecyclerObjectClickListener<DoneWorkListModel>
        , BaseViewHolder<DoneWorkListModel, OnRecyclerObjectClickListener<DoneWorkListModel>>>{

    public WorkDoneableAdapter(Context context) {
        super(context);
    }
    ...
    public BaseViewHolder<DoneWorkListModel, OnRecyclerObjectClickListener<DoneWorkListModel>> onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        switch (viewType) {
            case ITEM:
            default:
                return new WorkDoneableViewHolder(inflate(R.layout.item_worker_work_01, parent));
            case LOADING:
